...
def build_http_get_message_from_url(
    domain: str, path: str, headers: dict[str, str] = None
):
...
if __name__ == "__main__":
    ...
    server_nodes = [
        ServerNode(
            ("localhost", 8080 + i), registry_address=("localhost", registry_port)
        )
        for i in range(10)
    ]
    ...
    response = tor_client.send_http_message(
        build_http_get_message_from_url(auth_server_host, "/private")
    )
    ...
    response = tor_client.send_http_message(
        build_http_post_message_from_url(
            auth_server_host,
            "/auth",
            json.dumps({"username": "gaspard", "password": "mypassword"}),
        )
    )
    raw_response = extract_data_from_http_raw_response(response)
    ...
    response = tor_client.send_http_message(
        build_http_get_message_from_url(
            auth_server_host, "/private", headers={"Token": json_response["token"]}
        )
    )
    raw_response = extract_data_from_http_raw_response(response)
    json_response = json.loads(raw_response.body)
    ...
    time.sleep(1)
    print(
        "Now demonstrating HTTPS request over our home-made tor network which is built over http."
    )
    time.sleep(1)
    print("Sounds crazy ?")
    time.sleep(1)
    ...
    print(
        "Once that done, change your browser settings to use the proxy and try to access a website"
    )
    input(
