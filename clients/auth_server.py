...
from domain.crypto import CryptoContainer
...
class AuthServerHTTPHandler(socketserver.ThreadingMixIn, BaseHTTPRequestHandler):
        ...
        if self.path[:8] == "/private":
            ...
            if token and token in self.user_tokens:
                ...
                self.wfile.write(
                    json.dumps(
                        {
                            "data": f"Secret for your eyes only ;) Hello {self.user_tokens[token]}"
                        }
                    ).encode("utf-8")
                )
            else:
            ...
            self.wfile.write(json.dumps({"code": 404, "message": "Not found"}).encode())
    ...
    def do_POST(self):
        ...
        if self.path == "/auth":
            ...
            if (
                body_json.get("username", None) == "gaspard"
                and body_json.get("password", None) == "mypassword"
            ):
